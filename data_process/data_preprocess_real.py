...
time_gap_thres = {'pos': 0.008, 'imu': 0.015, 'uwb': 0.015, 'cam': 0.03, 'world_pos': 0.008} # cam set to 10s for consecutive infer, 0.03 for non-consecutive
# graph level
graph_ind = -1
g_graphid_list = []
g_timestamp_list = []
...
def validate_data(list_k, line_index_k, t_now):
    ...
    if len(line_k['uwb']) != 1+2*(swarm_num-1):
        print("uwb_line not complete: ", len(line_k['uwb']))
        return False
