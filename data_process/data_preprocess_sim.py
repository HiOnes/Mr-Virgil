...
train_dataset = ['sim0', 'sim1', 'sim2', 'sim3', 'sim4', 'sim5', 'sim6', 'sim7']
val_dataset = ['sim8', 'sim9']

dataset = {'train': train_dataset, 'val': val_dataset}
mode = 'val'

'''
swarm_num | max_cam_num
    4     |     8
    8     |     12
    12    |     16
    16    |     20
...
swarm_num = 16
max_cam_num = 20
scene_path = "sim_16robots/" # should match the swarm_num
...
time_gap_thres = {'pos': 0.03, 'imu': 0.01, 'uwb': 0.03, 'cam': 0.05, 'world_pos': 0.03} # cam set to 10s for consecutive infer, 0.05 for non-consecutive
# graph level
graph_ind = -1
g_graphid_list = []
g_timestamp_list = []
...
def process(graph_ind, k, data_list, line_index, t_now):
    ...
    if abs(t_now-float(cam_line[0])) > 0.05:
